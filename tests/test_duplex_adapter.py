...
async def test_words_for_sound_already_played_join_their_burst_during_grace(duplex) -> None:
    """The burst's sound has ended by the time its last words arrive; within the grace window they
    still join the generation that carried them, and never anchor the next utterance."""
    fake, session, generations = duplex
    fake.push(0.001, count=20)
    fake.push(0.3, count=3)
    await _settle()
    ...
    await _settle()
    assert len(generations) == 2
    (first,) = await _chunks(generations[1])
    assert first == "Next."
    assert isinstance(first, TimedString) and first.start_time == pytest.approx(0.0)


async def test_late_words_do_not_join_the_burst_that_is_already_sounding(duplex, caplog) -> None:
    """Once the next utterance sounds, words for the finished one cannot be placed on audio that
    is still playing; they are omitted rather than published without their sound."""
    fake, _session, generations = duplex
    fake.push(0.001, count=20)
    fake.push(0.3, count=3)
    await _settle()
    ...
    assert any(getattr(r, "reason", None) == "finalized_audio" for r in caplog.records)
